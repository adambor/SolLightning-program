...
async function execute(accounts: IXAccounts, _withdrawAmount?: BN): Promise<SignatureResult> {
...
describe("swap-program: Withdraw", () => {
    ...
    parallelTest.it("UserAccount with not enough funds", async () => {
        ...
        const result = await execute(accs, withdrawTooMuch);
    ...
    parallelTest.it("UserAccount of other signer", async () => {
        ...
        accs.userData = SwapUserVault(otherSigner.publicKey, accs.mint);

        await getInitializedUserData(otherSigner, accs.mintData, depositAmount);
    ...
    parallelTest.it("UserAccount of other mint", async () => {
        ...
        const otherMint = await getNewMint();
        accs.userData = SwapUserVault(accs.signer.publicKey, otherMint.mint);

        await getInitializedUserData(accs.signer, otherMint, depositAmount);
    ...
    parallelTest.it("UserAccount of other signer & mint", async () => {
        ...
        const otherMint = await getNewMint();
        accs.userData = SwapUserVault(otherSigner.publicKey, otherMint.mint);

        await getInitializedUserData(otherSigner, otherMint, depositAmount);
